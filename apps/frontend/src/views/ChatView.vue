...
.chat-view__conversation > header { align-items: center; background: linear-gradient(180deg, var(--surface-raised), rgb(255 255 255 / 92%)); border-bottom: 1px solid var(--line); display: flex; justify-content: space-between; min-height: 3.9rem; padding: 0.75rem clamp(1rem, 3vw, 2rem); }
...
.chat-view :deep(button:focus), .chat-view :deep(a:focus), .chat-view :deep(input:focus), .chat-view :deep(select:focus), .chat-view :deep(textarea:focus) { outline: none; } .chat-view :deep(button:focus-visible), .chat-view :deep(a:focus-visible), .chat-view :deep(input:focus-visible), .chat-view :deep(select:focus-visible), .chat-view :deep(textarea:focus-visible) { outline: 2px solid var(--accent); outline-offset: 2px; }
