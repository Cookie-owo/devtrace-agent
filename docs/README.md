...
DevPilot 是一个面向软件研发、测试与运维场景的本地优先 AI Agent 工作台。平台统一提供 CI/Test Failure Diagnosis、测试验证、受控修复、知识库问答、实时对话和 AIOps 诊断能力：Agent 读取测试日志、Git 变更和源码，构建可追溯证据链，输出根因报告，并在用户明确触发后，在隔离工作区中生成受控 Patch、运行白名单测试完成验证。
